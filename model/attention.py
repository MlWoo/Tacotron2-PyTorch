...
class LocationAttention_wrong(nn.Module):
    def __init__(self, dim_key, dim_query, dim_attention, num_location_features, attention_kernel_size=31,
                 smoothing=True, cumulate=True):
        super(LocationAttention_wrong, self).__init__()

        self.W = nn.Linear(in_features=dim_query, out_features=dim_attention)
        self.V = nn.Linear(in_features=dim_key, out_features=dim_attention, bias=False)
        self.U = nn.Conv1d(in_channels=num_location_features, out_channels=dim_attention, kernel_size=1, padding=0, bias=False)
        self.conv_f = nn.Conv1d(in_channels=1, out_channels=num_location_features, kernel_size=attention_kernel_size,
                                padding=attention_kernel_size//2)
        self.w = nn.Conv1d(in_channels=dim_attention, out_channels=1,  kernel_size=1, padding=0, bias=False)
        self.score_mask_value = -float('inf')
        self.smoothing = smoothing
        self.cumulate = cumulate

    def init_state(self, memory, mask):
        self.key_attention = self.V(memory).permute(1, 2, 0) # [B x Ca x T]
    ...
    def score(self, query, probability):
        ...
        :return:
        """
        query_attention = self.W(query).permute(1, 2, 0) # [B x Ca x 1]
        location_value = self.conv_f(probability)  # [B x Cl x T]
        location_attention = self.U(location_value)  # [B x Ca x T]
        return self.w(torch.tanh(self.key_attention + location_attention + query_attention))  # [B x 1 x T]
    ...
    def forward(self, query_input, state):
        ...
        :return:
        """
        energies = self.score(query_input, state)  # [B x 1 x T]
        if self.mask is not None:
            energies.data.masked_fill_(self.mask, self.score_mask_value)
        if self.smoothing:
            probability = self.softmax_smoothing(energies, dim=2)  # [B x 1 x T]
        else:
            probability = torch.nn.functional.softmax(energies, dim=2)  # [B x 1 x T]
        if self.cumulate:
            next_state = probability + state
        else:
            next_state = probability
        context = torch.bmm(probability, self.memory)  # [B x 1 x Ck]
        context_tbc = context.permute(1, 0, 2)         # [1 x B x Ck]
